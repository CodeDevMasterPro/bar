                        ...
                        <a-card :loading="loading" size="small">
                            <a-card-meta>
                                ...
                                <div class="meta-content" slot="description">
                                    <h4 style="background: #e6fffb">分类ID：</h4>
                                    <p style="text-indent: 2em;margin-bottom: 5px">{{ item.id }}</p>
                                </div>
                                <div class="meta-content" slot="description">
                                    <h4 style="background:  #b5f5ec">分类名称：</h4>
                                    <p style="text-indent: 2em;margin-bottom: 5px"> {{ item.name }}</p>
                                </div>
                                <div class="meta-content" slot="description">
                                    <h4 style="background: #87e8de">创建时间：</h4>
                                    <p style="text-indent: 2em;margin-bottom: 5px"> {{ item.time }}</p>
                                </div>
                                <div class="meta-content" slot="description">
                                    <h4 style="background: #5cdbd3">是否启用：</h4>
                                    <p style="text-indent: 2em;margin-bottom: 5px"> {{ item.enabled }}</p>
                                </div>
                                <div class="meta-content" slot="description">
                ...
                >
                    <a-card :hoverable="true" size="small">
                        <a-form :form="form" @submit="handleSubmit">
                            ...
                            <a-form-item label="分类ID" style="margin-bottom: 0" v-if="disable">
                                <a-input :disabled="true"
                                         v-decorator="['id', { rules: [{ required: true, message: 'Please input your note!' }] }]"/>
                            </a-form-item>
                            <a-form-item label="分类名称" style="margin-bottom: 0">
                                <a-input
                                        v-decorator="['name', { rules: [{ required: true, message: 'Please input your note!' }] }]"/>
                            </a-form-item>
                            <a-form-item label="是否启用" style="margin-bottom: 0">
                                <a-select
                                        v-decorator="['enabled', { rules: [{ required: true, message: 'Please input your note!' }] }]"
                                >
